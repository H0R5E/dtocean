...
from ..modules.electrical import ElectricalInterface
...
def get_constraints(
    data: dict[str, Any],
) -> tuple[Electrical, list[LineString]]:
    ...
    if elec.status < 0:
        ...
        elec.grid, constrained_polygons, constrained_lines = grid_processing(
            elec.site_data, elec.export_data, elec.options
        )
