...
class PyPower:
    ...
    def branch_count(self) -> int:
        ...
        n_voltage_levels = len(
            set([self.export_voltage, self.array_voltage, self.device_voltage])
        )
    ...
    def set_generators(self) -> np.ndarray | None:
        ...
        gen_data[:, 0][1 : self.n_devices + 1] = range(
            connect_bus, self.n_bus + 1
        )
    ...
    def device_to_device_fn(
        ...
        while chain:
            if np.any(device_to_device[chain_step] > 0):
                ...
                # create branch
                z_device = z_matrix[chain_step + 1][
                    next_device + 1
                ]  # +1 substation
                branch_data[branch_count][:5] = [
                    branch_start,
    ...
    def transformer_impedance(
        ...
        transformer_base, transformer_pu_own_base = transformer_db[
            (transformer_db.v1.isin(voltages))
            & (transformer_db.v2.isin(voltages))
        ][["rating", "impedance"]].values[0]

        transformer_pu_system_base = (
            transformer_pu_own_base / 100.0 * (transformer_base / self.MVAb)
        )
...
class ComponentLoading:
    ...
    def check_component_loading(
        ...
        S_injection = [
            np.sqrt(np.square(P) + np.square(Q))
            for P, Q in zip(P_injection, Q_injection)
        ]  # result in MVA

        S_injection_branched = zip(
            pf_result["branch"][:, 1].astype(int), S_injection
        )
