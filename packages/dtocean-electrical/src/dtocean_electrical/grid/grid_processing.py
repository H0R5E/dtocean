...
def grid_processing(
    ...
    clipped_export, lease_polygon = clip_grid(
        site_data.bathymetry,
    ...
    grid = make_grid(
        site_data.bathymetry, clipped_export, network_graph, lease_polygon
    )

    grid.remove_exclusion_zones(exclusion_zones)

    constrained_lines = []
    if options.equipment_gradient_constraint is not None:
...
def clip_grid(
    ...
    static_coords = [(x, y) for x, y in zip(grid_df_static.x, grid_df_static.y)]
    static_multipoint = MultiPoint(static_coords)
    static_poly = static_multipoint.convex_hull

    if not isinstance(static_poly, Polygon):
        raise RuntimeError("Could not calculate enclosing polygon for grid")
    ...
    while True:
        ...
        clipped_coords = [
            (x, y) for x, y in zip(grid_df_clipped.x, grid_df_clipped.y)
        ]

        shared_coords = list(
            set(clipped_coords).intersection(set(static_coords))
        )
        ...
        if static_poly.area <= 0.0:
            errStr = (
                "No overlapping nodes were found between lease area "
                "and cable corridor."
            )
        ...
        static_poly = static_poly.buffer(-maxd / 2.0)
...
def make_graph(
    ...
    graph_dict_static = get_neighbours_distance(
        grid_df_static, id_indexed_grid_df, id_array, x_array, y_array, z_array
    )
    ...
    graph_dict_clipped = get_neighbours_distance(
        grid_df_clipped, id_indexed_grid_df, id_array, x_array, y_array, z_array
    )

    assert (
        set(graph_dict_static.keys()) & set(graph_dict_clipped.keys())
    ) == set()

    # Get dataframe of overlapping points
    overlap_df = pd.merge(grid_df_static, grid_df_clipped, on=["x", "y"])
...
def make_gradient_dict(
    ...
    graph_dict_static = get_neighbours_gradient(
        grid_df_static,
        id_indexed_grid_df,
        id_array,
        x_array,
    ...
    graph_dict_clipped = get_neighbours_gradient(
        grid_df_clipped,
        id_indexed_grid_df,
        id_array,
        x_array,
...
def make_grid(
    ...
    grid_df_joined = pd.concat(
        [grid_df_static, grid_df_clipped],
        ignore_index=True,
    )

    # Initalise the Grid object
    grid = Grid(grid_df_joined, network_graph, lease_polygon)
...
def apply_equipment_constraints(
    ...
    if preselected:
        ...
        valid_installers = grid.check_equipment_soil_compatibility_site(
...
def get_metric_edges(
    ...
    for i, j in search_ij:
        ...
        if np.isnan(point_id):
            continue

        ij_point_coords = (x_array[i, j], y_array[i, j], z_array[i, j])
...
def weights_from_metrics(
    ...
    weight_dict = {
        index: {"weight": metric} for index, metric in zip(indices, metrics)
    }
...
def edge_length(p1: PointTuple, p2: PointTuple) -> float:
    ...
    edge_length = distance.euclidean(
