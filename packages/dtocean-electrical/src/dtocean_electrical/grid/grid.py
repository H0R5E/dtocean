...
class Grid:
    """Data structure for the grid. This is composed of a number of GridPoint
        ...
        Attributes:
            ...
            all_ids (pd.Series): List of all point ids.
            all_x (pd.Series): List of all point x coordinates.
            all_y (pd.Series): List of all point y coordinates.
            graph : Networkx graph object of the grid.
    ...
    #############
    # For removal
    #        jetting_graph : Networkx graph object of the grid filtered for
    #            the jetting installation tool.
    #        ploughing_graph : Networkx graph object of the grid filtered
    #            for the ploughing installation tool.
    #        cutting_graph : Networkx graph object of the grid filtered for
    #            the cutting installation tool.
    #        dredging_graph :Networkx graph object of the grid filtered for
    ...
    def get_soil_coverage(self) -> dict[str, float]:
        ...
        for soil in self.soil_types:
            number_soil_points = self.grid_pd["layer 1 type"].value_counts()[
                soil
            ]
    ...
    def add_points_to_grid(self):
        ...
        all_points = {
            row.id: GridPoint(row.id, row) for _, row in self.grid_pd.iterrows()
        }
    ...
    def check_equipment_soil_compatibility(
        ...
        Attributes:
            tool_points (list): List of Point ids where the technique can
                install.
            updated_graph : Networkx graph object with points removed.
        ...
        points = self.grid_pd[-self.grid_pd["layer 1 type"].isin(soil_list)].ix[
            :, "id"
        ]
        ...
        logMsg = (
            "Removing {} points containing incompatible soil types for "
            "installation technique {}"
        ).format(len(points), technique)
        module_logger.info(logMsg)
    ...
    def burial_protection_index(
        ...
        Attrbitues:
            ...
            points_to_remove (list): Points to be removed from the Networkx
                graph object.
            updated_graph : Networkx graph object updated with points
                removed.
    ...
    def _make_lines(
        ...
        for edge in edge_id_list:
            ...
            line = LineString(
                [(start_row["x"], start_row["y"]), (end_row["x"], end_row["y"])]
            )
