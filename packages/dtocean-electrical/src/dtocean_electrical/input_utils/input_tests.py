...
def check_inputs(input_data: "Electrical") -> tuple[int, list[str]]:
    ...
    try:
        ...
        if machine_type not in ["floating", "fixed"]:
            errstatus -= 1
            errstr.append(
                "Machine type is not valid. Please specify: "
                "'floating' or 'fixed'"
            )
    ...
    try:
        ...
        if not 3000000.0 >= power >= 100000.0:
            errstatus -= 1
            errstr.append("Machine power does not fit: " "0.1 <= Poec (MW) < 3")
    ...
    try:
        ...
        if connection not in ["wet-mate", "dry-mate"]:
            errstatus -= 1
            errstr.append(
                "Connection type is not valid. Please enter: "
    ...
    array_power_mw = (
        input_data.array_data.n_devices
        * input_data.array_data.machine_data.power
        / 1e6
    )

    if array_power_mw < 0.0:
        errstatus -= 1
    ...
    if input_data.options.user_umbilical:
        ...
        if (
            ...
            not in input_data.database.dynamic_cable.id.tolist()
        ):
            errstatus -= 1
            errstr.append(
                "Selected umbilical cable does not exist."
        ...
        # all others are warnings
            ...
            if (
                ...
                msg = (
                    "Selected umbilical cable voltage rating, {} kV, is not"
            ...
            if (
                ...
                msg = (
                    "Selected umbilical cable current rating, {} A, is less"
