...
class Electrical:
    ...
    def __init__(
        ...
        array_data: ElectricalArrayData,
    ...
    def run_module(self, plot=False, iterate_tools=False):
        ...
        if self.status < 0:
            ...
            if self.options.installation_tool:
                ...
                try:
                    msg = (
                        "Checking cable routes for installation"
                    ...
                    msg = (
                        "Could not create cable routes for selected "
                        "installation tool: {}. Trying with unfiltered "
                        "seabed.".format(self.options.installation_tool)
                    )
                    ...
                    try:
                        ...
                        msg = (
                            "Solution found for installation tool: "
                            "{}".format(best_tool)
                        )
                        module_logger.info(msg)
                ...
                if iterate_tools:
                    ...
                    multi_tool_results = self.iterate_multi_tools(
                        electrical_design, all_tools
                    )

                else:
                    tool_order = select_installation_tool(
                        self.grid.soil_coverage,
                        all_tools,
                        self.options.installation_rates,
                    )

                    multi_tool_results = self.iterate_multi_tools(
                        electrical_design, tool_order, ordered=True
                    )

                # unpackage results
                solution = multi_tool_results["flag"]
                all_solutions_cost: dict[str, float] = multi_tool_results[
                    "cost"
                ]
                all_solutions_data: dict[str, Network] = multi_tool_results[
                    "data"
                ]

                if solution:
                    # compare tool solutions and select best for return
                    clean_solutions: dict[str, float] = {
                        k: v for k, v in all_solutions_cost.items() if v
                    }

                    best_tool = min(
                        clean_solutions, key=lambda k: clean_solutions[k]
                    )
                    ...
                    errStr = (
                        "Could not create cable routes. " "Check exclusions."
                    )
    ...
    def iterate_multi_tools(
        ...
        all_solutions_data: dict[str, Network | None] = {
            key: None for key in tools
        }
        solution_found = False

        for tool in tools:
            msg = "Checking cable routes for installation" " tool: {}".format(
                tool
            )
            module_logger.info(msg)
            ...
            try:
                ...
                msg = "Solution found for installation tool: " "{}".format(tool)
                module_logger.info(msg)
                ...
                msg = (
                    "Could not find cable routes for "
                    "installation tool: {}.".format(tool)
                )
                module_logger.warning(msg)

        final_result = {
            "cost": all_solutions_cost,
